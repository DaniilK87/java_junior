        ...
        public void run() {
            ...
            while (true) {
                ...
                if (SendMessageRequest.TYPE.equals(type)) {
                    ...
                    final BroadcastMessageRequest broadcastMessageRequest;
                    try {
                        broadcastMessageRequest = objectMapper.reader().readValue(msgFromClient, BroadcastMessageRequest.class);
                    } catch (IOException e) {
                        sendMessage("Не удалось прочитать сообщение SendMessageToAllClientRequest: " + e.getMessage());
                        continue;
                    }
                    sendMessageToAllClients(broadcastMessageRequest.getMessage());
